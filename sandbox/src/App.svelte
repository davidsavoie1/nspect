<script context="module">
  ...
  import castValueByType, { attribute as attributeSpec } from "./attributeSpec";
...
<script>
  import ChoicesTable from "./ChoicesTable.svelte";
  ...
  let value = {
    ...
    type: "STRING",
    allowInput: false,
    defaultValue: "A.M MIDATECH, Sherbrooke",
    fractions: null,
    choicesSort: {
      by: "value",
      direction: 1,
    },
    // choices: Array.from({ length: 100 }, (_, idx) => ({
    //   _id: idx.toString(),
    //   value: idx,
    //   nomenclatureValue: idx.toString(),
    // })),
    choices: [
      {
  ...
  $: if (!value.allowInput && !value.choices?.length) value.allowInput = true;

  $: updateChoicesByType(type);

  function updateChoicesByType(type) {
    if (!value.choices) return;

    value.choices = value.choices.map((ch) => ({
      ...ch,
      value: castValueByType(ch.value, type),
    }));
  }
  ...
  $: inspector = s.nspector({
    latent: true,
    // ensure: { foo: 1 },
    // selection: { name: 1, fractions: 1 },
    spec,
    submit: (v) => console.log("Submitted", v),
    to: (res) => (inspection = res),
  });
...
>
  <Nspector res={inspection.name} let:invalid let:message>
    <label class="block space-y-1">
      <div class={clsLabel}>
        Nom
        <FieldError {message} el="span" />
      </div>

      <input
        class={clsInputMaybeError(invalid)}
        type="text"
  ...
  <Nspector res={inspection.abbr} let:invalid let:message>
    <label class="block space-y-1">
      <div class={clsLabel}>
        Abbréviation
        <FieldError {message} el="span" />
  ...
  <Nspector res={inspection.type} let:invalid let:message>
    <label class="block space-y-1">
      <div class={clsLabel}>
        Type
        <FieldError {message} el="span" />
  ...
  <Nspector res={inspection.allowInput} let:invalid let:message>
    <label class="block space-y-1">
      <div class={clsLabel}>
        Saisie?
        <FieldError {message} el="span" />
      ...
      <select
        class={clsInputMaybeError(invalid)}
        disabled={!value?.choices?.length}
        bind:value={value.allowInput}
      >
  ...
  <Nspector res={inspection.defaultValue} let:invalid let:message>
    <label class="block space-y-1">
      <div class={clsLabel}>
        Valeur par défaut
        <FieldError {message} el="span" />
      ...
      {#if value.choices?.length > 0}
        ...
        >
          {#each value.choices as choice (choice._id)}
  ...
  <ChoicesTable
    bind:choices={value.choices}
    {type}
    inspection={inspection.choices}
  />

  <!-- <label class="block space-y-1">
    <span class={clsLabel}>Foo</span>
    <input
      class={clsInputMaybeError(s.explain(inspection.foo).invalid)}
      type="text"
      bind:value={value.foo}
      on:blur={() => inspector.activate("foo")}
    />
    <Nspector res={inspection.foo} name="Foo" />
  </label> -->

  <!-- <label class="block space-y-1">
    <span class={clsLabel}>LA Réponse</span>
    <input
      class={clsInputMaybeError(s.explain(inspection.ansToTheUniverse).invalid)}
      type="number"
      bind:value={value.ansToTheUniverse}
      on:blur={() => inspector.activate("ansToTheUniverse")}
    />
    <Nspector res={inspection.ansToTheUniverse} name="LA Réponse" />
  </label> -->

  <!-- <Nspector res={inspection} name="La valeur" /> -->

  <Nspector res={inspection} let:invalid>
    <button
  ...
  <!-- <button class={clsBtnPrimary} type="submit"> Soumettre </button> -->
  ...
  <button class={clsBtn} type="button" on:click={() => console.log(inspection)}>
    Log
  </button>
</form>
