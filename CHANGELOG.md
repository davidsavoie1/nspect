...
- Add `flex` spec definition option. Accepts a function of shape `(value, getFrom) => spec` to generate a spec at runtime based on the value to validate. Enables recursion, dispatch by type and other complex patterns.
- Add `inspect` main API function that validates an entire value without stopping at the first invalid. In case of a collection spec, returns another collection with same shape as the value, but with `undefined` when a predicate validation passes or the predicate error otherwise. Allows mapping complete value and its validation results directly. Contrary to other main API functions, always returns a promise (no async variation).
- `inspect` collection values have own error and list of errors attached with symbols. They can be retrieved with `ownErrors` and `getErrors` respectively.
- `getShape` function returns an inspection-like coll of the exact shape as the argument value initialized with all `undefined` values. Useful to create an initial value before the async inspection returns.
- Add `pass` function that returns a boolean indicating if an `inspect` result has errors.
- `required` can accept a function of shape `(value, getFrom) => required` so that required keys can be based on the value at runtime.
